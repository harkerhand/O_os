use core::{alloc::GlobalAlloc, mem, ptr::NonNull};
...
unsafe impl GlobalAlloc for SimpleAllocator {
    ...
    unsafe fn alloc(&self, layout: core::alloc::Layout) -> *mut u8 {
        ...
        loop {
            ...
            while let Some(node) = curr {
                ...
                if alloc_end <= block_end {
                    ...
                    if prefix >= MIN_BLOCK_SIZE {
                        Self::insert_free_block(&mut head, block_start, prefix);
                    }
                    let suffix_final = block_end - alloc_end;
                    if suffix_final >= MIN_BLOCK_SIZE {
                        Self::insert_free_block(&mut head, alloc_end, suffix_final);
                    }
            ...
            if !Self::request_more_and_insert(&mut head, need_total + align) {
